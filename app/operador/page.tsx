...
export default function InteractiveTabletPage() {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-slate-200 flex flex-col font-sans relative">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button
                            ...
                            className="h-12 bg-teal-900 hover:bg-teal-800 text-teal-100 border border-teal-700 font-bold gap-2 flex"
                        >
            ...
            {/* 1.5. STATION ANDON NOTIFICATION BAR (FAROL LOCAL) */}
            {selectedEstacaoId && areaAndonStatus.some(a => a.isCurrent && a.hasAndon) && (
                <div className="w-full py-2.5 px-6 shadow-[inset_0_4px_10px_rgba(0,0,0,0.5)] flex items-center justify-center gap-4 border-b border-red-900/50 transition-colors duration-1000 z-10 shrink-0 select-none bg-gradient-to-r from-slate-900 via-red-950/60 to-slate-900">
                    <div className="flex items-center gap-2 mr-2 shrink-0 border-r border-red-900/50 pr-4">
                        <div className="w-3 h-3 rounded-full bg-red-500 animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.8)]"></div>
                        <span className="text-xs font-black tracking-widest uppercase text-red-500">Alerta Ativo</span>
                    </div>
                    {areaAndonStatus.filter(a => a.isCurrent && a.hasAndon).map(station => (
                        <div key={station.id} className="flex items-center gap-4 px-6 py-2 rounded-full border border-red-500/80 bg-red-950/90 shadow-[0_0_20px_rgba(220,38,38,0.3)] min-w-[200px] justify-center">
                            <div className="relative flex h-5 w-5 shrink-0">
                                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                                <span className="relative inline-flex rounded-full h-5 w-5 bg-red-600 border border-red-400 shadow-[0_0_15px_rgba(239,68,68,1)]"></span>
                            </div>
                            
                            <div className="flex flex-col justify-center text-center">
                                <span className="text-sm font-black uppercase tracking-widest leading-none text-red-300">
                                    A aguardar assistência técnica
                                </span>
                                <span className="text-xs text-red-100 font-bold uppercase truncate max-w-[250px] leading-tight mt-1 opacity-90">
