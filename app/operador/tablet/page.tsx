...
import { getStationOperators, getStationChecklist } from './actions';
...
export default function InteractiveTabletPage() {
    ...
    const handleAndonTrigger = async () => {
        if (!selectedEstacaoId && !causadoraEstacaoId) return;
        setLcdLine1('ENVIAR ANDON...');
        setLcdLine2('Aguarde...');

        const res = await dispararAlertaAndon(causadoraEstacaoId || selectedEstacaoId, andonType, andonDesc);
        if (res.success) {
            setLcdLine1('ANDON REGISTADO!');
            setLcdLine2('Lider Notificado');
            ...
            <header className="bg-slate-900 border-b border-slate-800 p-4 flex flex-col gap-4 shadow-xl z-20">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <Button onClick={() => setIsAndonModalOpen(true)} className="bg-red-600 hover:bg-red-700 text-white shadow-lg shadow-red-900/20 font-bold hidden md:flex">
                            <AlertTriangle className="w-4 h-4 mr-2" />
                            Andon Help
                        </Button>
                        <Button className="bg-amber-500 hover:bg-amber-600 text-white shadow-lg shadow-amber-900/20 font-bold hidden md:flex">
                            <Lightbulb className="w-4 h-4 mr-2" />
                            Andon Kaizen
                        </Button>
                        <Button className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 hidden md:flex">
            ...
            <Dialog open={isAndonModalOpen} onOpenChange={setIsAndonModalOpen}>
                <DialogContent className="bg-slate-900 text-white border-slate-800 sm:max-w-[425px]">
                    ...
                    <DialogFooter>
                        ...
                        <Button className="bg-red-600 hover:bg-red-700 text-white font-bold" onClick={handleAndonTrigger}>
                            Disparar Alerta
                        </Button>
                    </DialogFooter>
                </DialogContent>
