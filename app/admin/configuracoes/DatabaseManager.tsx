...
import { importDataBulkStringified } from "./actions";
...
export function DatabaseManager() {
  ...
  const handleImportData = async (
    ...
    try {
      ...
      // raw: false converts Excel custom date/number objects to simple strings or plain javascript Numbers
      const rawData: any[] = XLSX.utils.sheet_to_json(ws, { defval: null, raw: false });

      if (rawData.length === 0) {
        throw new Error("O Excel fornecido está completamente vazio pós-conversão.");
      }
      
      setActionLog({ type: "info", msg: `A limpar e organizar ${rawData.length} linhas de forma massiva via Administração...` });
      ...
      // Pass as stringified payload to override NextJS default 1MB Server Action Array Validation crash
      const response = await importDataBulkStringified(tableName, JSON.stringify(jsonData));
