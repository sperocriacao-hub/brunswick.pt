...
export function Sidebar({
  ...
  userName?: string;
  notificacoesFormacaoCount?: number;
}) {
  const pathname = usePathname();
  const [isOpen, setIsOpen] = useState(false);
  const [showTakeover, setShowTakeover] = useState(true); // Controla o modal takeover
  ...
  const hasAccess = (modulePath: string) => {
    ...
    // 2. Dashboard principal, Smart Action Hub e My Gemba Hub são padrão para quem acede ao Backoffice.
    if (modulePath === "/admin" || modulePath === "/admin/melhoria-continua" || modulePath === "/admin/producao/gemba") return true;
          ...
          <div className="flex flex-col gap-1">
            ...
            {(hasAccess("/admin/rh/produtividade") || hasAccess("/admin/rh") || hasAccess("/admin/rh/iluo") || hasAccess("/admin/rh/avaliacoes") || hasAccess("/admin/rh/avaliacoes-lideranca") || hasAccess("/admin/rh/assiduidade")) && (
              <>
                    ...
                    <nav className="flex flex-col gap-1 mb-6">
                      {hasAccess("/admin/rh/formacoes") && (
                        <Link
                          ...
                          className={`flex items-center justify-between rounded-md px-3 py-2 transition-all font-medium ${pathname === "/admin/rh/formacoes" ? "bg-blue-800 text-white shadow-sm border border-transparent" : "text-blue-100 hover:bg-blue-800/50 hover:text-white"}`}
                        >
                ...
                {(hasAccess("/admin/lean/kaizen") || hasAccess("/admin/lean/gemba") || hasAccess("/admin/lean/acoes") || hasAccess("/admin/lean/5s/setup") || hasAccess("/admin/lean/5s")) && (
                  <>
                    ...
                    <nav className="flex flex-col gap-1 mb-6">
                      ...
                      {hasAccess("/admin/lean/5s") && (
                        <Link
                          onClick={() => setIsOpen(false)}
                          href="/admin/lean/5s"
                          className={`flex items-center gap-3 rounded-md px-3 py-2 transition-all font-medium ${pathname === "/admin/lean/5s" ? "bg-amber-600 text-white shadow-sm border border-transparent" : "text-blue-100 hover:bg-blue-800/50 hover:text-white"}`}
                        >
                          <Activity size={18} className={pathname === "/admin/lean/5s" ? "text-white" : "text-amber-400"} />
          ...
          <Link href="/admin/perfil" className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-blue-800/50 rounded-lg transition-colors border border-transparent hover:border-[rgba(255,255,255,0.1)] w-full">
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-sm font-bold text-white truncate">
                {userName || userEmail?.split("@")[0] || "Administrador"}
              </span>
