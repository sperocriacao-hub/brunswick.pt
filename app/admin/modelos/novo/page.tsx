...
import { criarModeloCompleto, CriarModeloInput, InMetaHH } from "./actions";
...
// Removido o mock ESTACOES para utilizar chamadas em tempo real à base de dados
...
export default function NovoModeloPage() {
  ...
  const [linhasProducao, setLinhasProducao] = useState<
    { id: string; descricao_linha: string }[]
  >([]);
  const [estacoesReais, setEstacoesReais] = useState<
    { id: string; nome_estacao: string }[]
  >([]);
  const [areasFabrica, setAreasFabrica] = useState<
    { id: string; nome_area: string }[]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (linhasData) setLinhasProducao(linhasData);

      const { data: estacoesData } = await supabase
        .from("estacoes")
        .select("id, nome_estacao")
        .order("nome_estacao");
      if (estacoesData) setEstacoesReais(estacoesData);

      const { data: areasData } = await supabase
        .from("areas_fabrica")
        .select("id, nome_area")
        .order("ordem_sequencial");
      if (areasData) setAreasFabrica(areasData);
    };
    fetchData();
  }, [supabase]);
  ...
  const updateTarefaGeral = (
    id: string,
    field: keyof Tarefa,
    value: string,
  ) => {
    setTarefasGerais(
      tarefasGerais.map((t) => (t.id === id ? { ...t, [field]: value } : t)),
    );
  };
  const removeTarefaGeral = (id: string) =>
    setTarefasGerais(tarefasGerais.filter((t) => t.id !== id));
  ...
  const removeOpcional = (id: string) =>
    setOpcionais(opcionais.filter((o) => o.id !== id));
  const saveCurrentOpcional = () => {
    if (currentOpcional) {
      ...
      if (exists) {
        setOpcionais(
          opcionais.map((o) =>
            o.id === currentOpcional.id ? currentOpcional : o,
          ),
        );
      } else {
  ...
  const updateTarefaOpcional = (
    ...
    if (currentOpcional) {
      setCurrentOpcional({
        ...currentOpcional,
        tarefas: currentOpcional.tarefas.map((t) =>
    ...
    try {
      ...
      alert(
        (error as Error).message ||
          "Falha a comunicar com servidor. Verifique se o Bucket existe no Supabase.",
      );
      if (contextoId === "geral")
        updateTarefaGeral(tarefaId, "imagem_url", "Erro ❌");
  ...
  const handleSaveModelo = async () => {
    if (!nomeModelo || !modelYear) {
      alert(
        "Por favor, preencha o Nome do Modelo e o Ano do Modelo (Model Year).",
      );
      return;
    }
    ...
    try {
      ...
      alert(
        "Modelo de Embarcação registado na Base de Dados com todas as dependências M.E.S!",
      );
  ...
  return (
    <div
      className="container mt-8 animate-fade-in dashboard-layout"
      style={{ display: "block" }}
    >
      <header className="flex justify-between items-center mb-8">
        <div>
          ...
          <p style={{ color: "rgba(255,255,255,0.7)", marginTop: "0.25rem" }}>
            Crie um novo Modelo, Partes, Roteiros e Opcionais.
          </p>
        </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {meta.tipo_alvo === 'AREA_LINHA' && (
                        <select className="form-control" value={meta.linha_id} onChange={e => updateMetaHH(meta.id!, 'linha_id', e.target.value)}>
                          <option value="">-- Selecione a Linha --</option>
                          {linhasProducao.map(l => <option key={l.id} value={l.id}>Linha {l.descricao_linha}</option>)}
                        </select>
                      )}
                      {meta.tipo_alvo === 'ESTACAO' && (
                        <select className="form-control" value={meta.estacao_id} onChange={e => updateMetaHH(meta.id!, 'estacao_id', e.target.value)}>
                          ...
                          {estacoesReais.map(e => <option key={e.id} value={e.id}>{e.nome_estacao}</option>)}
                        </select>
                      )}
                    </div>
                  </td>
      ...
      <section className="glass-panel p-6 mb-8 animate-delay-2">
        <div className="flex justify-between items-center mb-6">
          <h2 style={{ fontSize: "1.25rem", color: "var(--primary)" }}>
            Tarefas de Produção Comuns (Geral)
          </h2>
            ...
            <tbody>
              {tarefasGerais.length === 0 && (
                <tr>
                  ...
                  >
                    As tarefas gerais base darão corpo ao Roteiro por defeito do
                    Barco.
                  </td>
                </tr>
              )}
                ...
                <tr key={tarefa.id}>
                  <td>
                    <input
                      ...
                      value={tarefa.ordem}
                      onChange={(e) =>
                        updateTarefaGeral(tarefa.id, "ordem", e.target.value)
                      }
                    />
                  </td>
                  <td>
                    <input
                      type="text"
                      className="form-control"
                      placeholder="Instrução exata para o operador..."
                  ...
                  <td style={{ textAlign: "center" }}>
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          handleFileUpload(e, tarefa.id, "geral")
                        }
                      />
                      <Upload
                        size={18}
                      ...
                      {tarefa.imagem_url && (
                        <span
                          style={{
                            fontSize: "0.65rem",
                            color: tarefa.imagem_url.includes("Erro")
                              ? "var(--danger)"
                              : "var(--secondary)",
                            marginTop: "4px",
                            maxWidth: "80px",
                          ...
                          title={tarefa.imagem_url}
                        >
                          {tarefa.imagem_url.startsWith("http")
                            ? "Anexado ✅"
                            : tarefa.imagem_url}
                        </span>
                      )}
                  ...
                  <td style={{ textAlign: "right" }}>
                    <button
                      ...
                      onClick={() => removeTarefaGeral(tarefa.id)}
                    >
                      <Trash2 size={18} />
                    </button>
                  </td>
      ...
      <section className="glass-panel p-6 mb-8 animate-delay-3">
        ...
        <div className="grid grid-cols-3 gap-6">
          {opcionais.length === 0 && (
            ...
            >
              Sem opcionais específicos configurados. (Ex: Ar Condicionado,
              Upgrade Estofos)
            </div>
          )}
          {opcionais.map((op) => (
            <div
              ...
              onClick={() => editOpcional(op)}
            >
              <div className="flex justify-between items-start mb-2">
                <h3 style={{ fontSize: "1.1rem" }}>
                  {op.nome_opcao || "Opcional Sem Nome"}
                </h3>
              ...
              >
                {op.descricao_opcao || "Sem descrição"}
              </p>
          ...
          <div className="modal-content animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="form-group">
                ...
                <input
                  type="text"
                  className="form-control"
                  value={currentOpcional.nome_opcao}
                  onChange={(e) =>
                    setCurrentOpcional({
                      ...currentOpcional,
                      nome_opcao: e.target.value,
                    })
                  }
                  placeholder="Ex: GPS Premium"
                />
              </div>
              <div className="form-group">
                ...
                <input
                  type="text"
                  className="form-control"
                  value={currentOpcional.descricao_opcao}
                  onChange={(e) =>
                    setCurrentOpcional({
                      ...currentOpcional,
                      descricao_opcao: e.target.value,
                    })
                  }
                />
              </div>
            ...
            <div className="flex justify-between items-center mb-4 mt-8">
              <h3 style={{ fontSize: "1.1rem", color: "var(--primary)" }}>
                Tarefas para construir este Opcional
              </h3>
              <button
                className="btn btn-outline"
                onClick={addTarefaToOpcional}
            ...
            <div className="table-container mb-8">
              <table className="table-premium" style={{ tableLayout: "fixed" }}>
                ...
                <tbody>
                  {currentOpcional.tarefas.length === 0 && (
                    <tr>
                      <td
                        colSpan={5}
                        style={{ textAlign: "center", opacity: 0.5 }}
                      >
                        Adiciona a primeira tarefa técnica deste opcional.
                      </td>
                    </tr>
                  )}
                    ...
                    <tr key={tarefa.id}>
                      ...
                      <td>
                        <input
                          ...
                          className="form-control"
                          value={tarefa.descricao}
                          placeholder="O que o operador 1 tem de fazer?"
                          onChange={(e) =>
                            updateTarefaOpcional(
                              tarefa.id,
                              "descricao",
                      ...
                      <td style={{ textAlign: "center" }}>
                        ...
                        >
                          <input
                            ...
                            disabled={isUploading}
                            onChange={(e) =>
                              handleFileUpload(e, tarefa.id, "opcional")
                            }
                          />
                          <Upload
                            size={16}
                      ...
                      <td style={{ textAlign: "right" }}>
                        <button
                          className="btn-icon danger"
                          onClick={() => removeTarefaOpcional(tarefa.id)}
                        >
                          <Trash2 size={16} />
                        </button>
                      </td>
            ...
            <div
              className="flex justify-end gap-4 border-t border-[var(--border)] pt-6"
              style={{ borderTop: "1px solid var(--border)" }}
            >
              <button
                className="btn btn-outline"
                onClick={() => {
                  removeOpcional(currentOpcional.id);
                  setIsModalOpen(false);
                }}
              ...
              <button className="btn btn-primary" onClick={saveCurrentOpcional}>
                Guardar na Lista
              </button>
            </div>
          </div>
