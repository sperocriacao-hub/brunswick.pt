...
export default function EditarOitodPage() {
    ...
    async function carregarBase() {
        setLoading(true);
        const res = await get8d(id);
        if (res.success && res.report) {
            setRnc(res.report.qualidade_rnc);
            setReportId(res.report.numero_8d);
            setStatusLabel(res.report.status);
            
            // Fill existing D8 fields
            setResponsavel(res.report.responsavel_nome || '');
            setD1(res.report.d1_equipa || '');
            setD2(res.report.d2_problema || res.report.qualidade_rnc?.descricao_problema || '');
            setD3(res.report.d3_contencao || '');
            setD4(res.report.d4_causa_raiz || '');
            setD5(res.report.d5_acao_permanente || '');
            setD6(res.report.d6_implementacao || '');
            setD7(res.report.d7_prevencao || '');
            setD8(res.report.d8_reconhecimento || '');
        } else {
            console.error("Falha a carregar relatório 8D:", res.error);
        }
        setLoading(false);
    }
    ...
    async function handleSave(newStatus?: string) {
        ...
        const res = await update8d(id, payload);
        if (res.success) {
            router.push('/admin/qualidade/rnc');
        } else {
            alert('Falha a atualizar relatório 8D: ' + res.error);
            setSubmitting(false);
        }
    }

    if (loading) return <div className="p-12 text-center text-slate-500 font-medium animate-pulse">A carregar registo do relatório 8D...</div>;
        ...
        <div className="p-8 space-y-8 max-w-[1000px] mx-auto animate-in fade-in duration-500 pb-32">
            <header className="flex items-start justify-between border-b border-slate-200 pb-4">
                ...
                <div className="flex flex-col gap-2">
                    <Button disabled={submitting} variant="outline" className="border-indigo-200 text-indigo-700 bg-indigo-50" onClick={() => handleSave()}>
                        <Save className="w-4 h-4 mr-2" /> Atualizar Registo
                    </Button>
                    {status !== 'Concluido' && (
                        <Button disabled={submitting} onClick={() => handleSave('Concluido')} className="bg-indigo-600 hover:bg-indigo-700 font-bold shadow-md">
                            {submitting ? <Loader2 className="animate-spin w-4 h-4 mr-2" /> : <CheckSquare className="w-4 h-4 mr-2" />}
                            Encerrar 8D (Concluir Análise)
                        </Button>
                    )}
                </div>
            </header>
