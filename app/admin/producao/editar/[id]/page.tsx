...
export default function EditarOrdemProducao({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const loadMasterData = async () => {
            ...
            if (res.success && res.data) {
                ...
                alert('Erro a carregar Master Data: ' + res.error);
            }
        };

        const loadOpData = async () => {
            if (!opId) return;
            const res = await getOrdemProducaoById(opId);
            if (res.success && res.data) {
                const op = res.data;
                setModeloSelecionado(op.modelo_id || '');
                setLinhaSelecionada(op.linha_id || '');
                setOpNumero(op.op_numero || '');
                setPp(op.pp_plan || '');
                setPo(op.po_compra || '');
                setHin(op.hin_hull_id || '');
                setNs(op.num_serie || '');
                setCliente(op.cliente || '');
                setPais(op.pais || '');
                setRegion(op.brand_region || '');
                setRfidToken(op.rfid_token || '');
                
                // Dates
                if (op.data_inicio) setDataInicio(op.data_inicio.split('T')[0] || op.data_inicio);
                if (op.data_fim) setDataFim(op.data_fim.split('T')[0] || op.data_fim);

                // Molds Engine
                setMoldeCascoId(op.molde_casco_id || '');
                setMoldeCobertaId(op.molde_coberta_id || '');
                setMoldeSmallPartsId(op.molde_small_parts_id || '');
                setMoldeLinerId(op.molde_liner_id || '');
                setRfidCasco(op.rfid_casco || '');
                setRfidCoberta(op.rfid_coberta || '');
                setRfidSmallParts(op.rfid_small_parts || '');
                setRfidLiner(op.rfid_liner || '');

                // Restore previous options
                if (op.ordens_producao_opcionais && Array.isArray(op.ordens_producao_opcionais)) {
                    setOpcionaisSelecionados(op.ordens_producao_opcionais.map((x: any) => x.opcional_id));
                }
            } else {
                alert('Erro a carregar Ordem de Produção: ' + res.error);
    ...
    const handleSubmit = async () => {
        ...
        const res = await atualizarOrdemProducao(opId, {
        ...
        if (res.success) {
            ...
            alert('Erro ao atualizar OP: ' + res.error);
        }
        ...
        <div className="container mx-auto mt-4 animate-fade-in max-w-7xl pb-20">
            <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-blue-900">Editar Ordem de Produção</h1>
                        <p className="text-muted-foreground mt-1">Correção manual de campos, métricas e rastreio de OPs.</p>
                    </div>
                </div>
                <Button className="bg-blue-600 hover:bg-blue-700 text-white shadow-md sm:w-auto w-full" onClick={handleSubmit} disabled={isSubmitting || isLoading}>
                    ...
                    {isSubmitting ? 'A guardar...' : 'Guardar Alterações (OP)'}
                </Button>
            </header>
