-- Migration 0092: Adicionar responsavel_id nas ações globais para ligação ao My Gemba Hub

ALTER TABLE public.central_acoes_globais ADD COLUMN IF NOT EXISTS responsavel_id UUID REFERENCES public.operadores(id) ON DELETE SET NULL;
ALTER TABLE public.lean_acoes ADD COLUMN IF NOT EXISTS responsavel_id UUID REFERENCES public.operadores(id) ON DELETE SET NULL;

-- Recriar a View Mestra com responsavel_id
...
SELECT 
    ...
    op.nome_operador AS responsavel_nome,
    ha.status,
    ha.created_at,
...
LEFT JOIN public.operadores op ON ha.responsavel_id = op.id
