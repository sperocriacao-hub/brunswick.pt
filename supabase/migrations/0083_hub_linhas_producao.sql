-- Migration 0083: Suporte a Linhas de Produção na Torre de Controlo (Smart Hub)

-- 1. Adicionar `linha_id` nas tabelas de ações para rastreio em contexto de Montagem
ALTER TABLE public.central_acoes_globais ADD COLUMN IF NOT EXISTS linha_id UUID REFERENCES public.linhas_producao(id) ON DELETE SET NULL;
ALTER TABLE public.lean_acoes ADD COLUMN IF NOT EXISTS linha_id UUID REFERENCES public.linhas_producao(id) ON DELETE SET NULL;
ALTER TABLE public.hst_acoes ADD COLUMN IF NOT EXISTS linha_id UUID REFERENCES public.linhas_producao(id) ON DELETE SET NULL;

-- 2. Recriar View Mestra
