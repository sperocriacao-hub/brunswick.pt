-- Migration 0093: Melhorar a visualização das ações HST no Hub Global
...
SELECT 
    ha.id,
    COALESCE(ho.tipo_ocorrencia, 'Ação HST') AS titulo,
    CASE 
        WHEN ha.descricao_acao LIKE 'Elaborar Relatório 8D%' THEN COALESCE(ho.descricao_evento, ha.descricao_acao)
        ELSE ha.descricao_acao
    END AS descricao,
...
LEFT JOIN public.hst_ocorrencias ho ON ha.ocorrencia_id = ho.id
