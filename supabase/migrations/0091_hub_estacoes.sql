-- Migration 0091: Adicionar suporte a estações na Torre de Controlo (Smart Action Hub)

-- 1. Adicionar estacao_id nas tabelas principais
ALTER TABLE public.central_acoes_globais ADD COLUMN IF NOT EXISTS estacao_id UUID REFERENCES public.estacoes(id) ON DELETE SET NULL;
ALTER TABLE public.lean_acoes ADD COLUMN IF NOT EXISTS estacao_id UUID REFERENCES public.estacoes(id) ON DELETE SET NULL;
ALTER TABLE public.hst_acoes ADD COLUMN IF NOT EXISTS estacao_id UUID REFERENCES public.estacoes(id) ON DELETE SET NULL;
ALTER TABLE public.lean_5s_acoes ADD COLUMN IF NOT EXISTS estacao_id UUID REFERENCES public.estacoes(id) ON DELETE SET NULL;

-- 2. Recriar a View Mestra com todas as colunas de localização (Área, Linha, Estação)
...
LEFT JOIN public.linhas_producao lp ON ha.linha_id = lp.id
LEFT JOIN public.estacoes e ON ha.estacao_id = e.id
